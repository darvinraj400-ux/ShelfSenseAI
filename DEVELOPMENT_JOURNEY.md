...
- **Cold-start problem:** No real historical sales data exists. Solution: generate **synthetic training data** by blending:
  - Real PriceCatcher market observations
  - Simulated cost prices and target margins
  - Generated stock levels and sales velocities
- **Model:** `RandomForestRegressor` from scikit-learn (100 trees, max_depth=10)
...
**Current state:** The ML model is trained on synthetic data generated by `scripts/train_pricing_model.py`. This solves the cold-start problem but does not learn from actual shop behavior.

**Planned enhancement:**
- After accumulating 6+ months of real sales data, retrain the model using actual transaction records.
- Implement a periodic retraining schedule (e.g., monthly) that blends synthetic + real data.
